...
---------------------------------
This module implements online linear classifier.
...
class PerceptronResult:
    ...
    def __repr__(self) -> str:
        return f"<PerceptronResult result={self.result!r} meta={self.meta}>"

def core_algorithm(data: Any, **kwargs: Any) -> PerceptronResult:
    """
    Core algorithm stub.
    Parameters
    ----------
    data : Any
        Input structure (online linear classifier).
    kwargs : Any
    ...
    return PerceptronResult(result={"acc": acc, "trace": trace}, meta=meta)
...
def pretty_print(result: PerceptronResult) -> str:
    """Create a human-readable summary."""
    acc = result.result.get("acc")
    pts = result.meta.get("trace_points")
    return f"Perceptron: acc={acc:.4f}, trace_points={pts}"
